...
while True:
    ...
    #if the user enters 'q' display the vertical histogram,
    #and end the program
    if end_cycle == "q":
        print("Progress",prog," | Trailer",trail," | Retriever",ret," |  Excluded",exc)
        #as long as the total count does not equal to o,
        #loop the program
        while total != 0:

            #a series of if blocks to print asterisks or spaces,
            #depending on the count of the respective progression outcomes,
            #and subtract 1 from each counter and the total counter as well
            if prog > 0:
                print("      *\t", end = '          ')
                prog -= 1
                total -= 1
        ...
        if tot_backup == 1:
            ...
            print(tot_backup,"outcomes in total")
        #exit the program once it prints out the histogram
        break
                
    #if the user enters 'y' go to the top of the loop,
    #and take user input for another set of values
    if end_cycle == "y":
        continue
